...
public abstract class ItemOldMekaSuitArmor extends ItemArmor implements IEnergizedItem,
    ...
    public ItemOldMekaSuitArmor(EntityEquipmentSlot slot) {
        super(EnumHelper.addArmorMaterial("MEKASUITOLD", "mekasuit", 0, new int[]{0, 0, 0, 0}, 0, SoundEvents.ITEM_ARMOR_EQUIP_GENERIC, 0), 3, slot);
        setMaxStackSize(1);
    ...
    private static float getDamageAbsorbed(EntityPlayer player, DamageSource source, float amount, @Nullable List<Runnable> energyUseCallbacks) {
        ...
        for (ItemStack stack : player.getArmorInventoryList()) {
            if (!stack.isEmpty() && stack.getItem() instanceof ItemOldMekaSuitArmor armor) {
    ...
    public void onArmorTick(World world, EntityPlayer player, ItemStack itemStack) {
        if (!world.isRemote) {
            if (itemStack.getItem() instanceof ItemOldMekaSuitArmor) {
                Shielding(itemStack);
            }
        }
    }
    ...
    private static class FoundArmorDetails {
        ...
        private final ItemOldMekaSuitArmor armor;
        private double energyContainer;

        public FoundArmorDetails(double energyContainer, ItemOldMekaSuitArmor armor) {
