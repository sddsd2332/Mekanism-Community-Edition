...
public interface IInventorySlot extends INBTSerializable<NBTTagCompound>, IContentsListener {
        ...
        if (isEmpty() || (sameType = ItemHandlerHelper.canItemStacksStack(getStack(), stack))) {
            ...
            if (action.execute()) {
                ...
                if (sameType) {
                    ...
                    // Note: this also will mark that the contents changed
                    setStack(stack.splitStack(toAdd));
                }
            }
            return stack.splitStack(stack.getCount() - toAdd);
        }
    ...
    default ItemStack extractItem(int amount, Action action, AutomationType automationType) {
        ...
        ItemStack toReturn = current.splitStack(amount);
        if (action.execute()) {
    ...
    default int setStackSize(int amount, Action action) {
        ...
        setStack(stack.splitStack(amount));
        return amount;
    }
