...
public interface ISecurityUtils {
   ...
   // ISecurityUtils INSTANCE = ServiceLoader.load(ISecurityUtils.class).findFirst().orElseThrow(() -> new IllegalStateException("No valid ServiceImpl for ISecurityUtils found"));
     ...
     * @see #canAccessObject(EntityPlayer, ISecurityObject)
     * @see #canAccessObject(UUID, ISecurityObject, boolean)
     * @see #canAccessOrDisplayError(EntityPlayer, ICapabilityProvider)
     */
    @Contract("_, null -> true")
    boolean canAccess(EntityPlayer player, @Nullable ICapabilityProvider provider);
     ...
     * @implNote This method assumes that if the security is {@link SecurityMode#TRUSTED} and there is a clientside player, then the player can access the
     * {@link ISecurityObject security object}. This is done because the list of trusted players is not currently synced to all clients.
     * @see #canAccess(EntityPlayer, ICapabilityProvider)
     * @see #canAccess(UUID, ICapabilityProvider, boolean)
     * @see #canAccessObject(UUID, ISecurityObject, boolean)
     * @see #canAccessOrDisplayError(EntityPlayer, ICapabilityProvider)
     */
    boolean canAccessObject(EntityPlayer player, ISecurityObject securityObject);
     ...
     */
    @Contract("_, null -> true")
    default boolean canAccessOrDisplayError(EntityPlayer player, @Nullable ICapabilityProvider provider) {
     ...
     */
    void displayNoAccess(EntityPlayer player);
     ...
     * on the client.
     */
    void addSecurityTooltip(ItemStack stack, List<String> tooltip);
}