...
public final class ModuleEnumData<TYPE extends Enum<TYPE>> implements ModuleConfigData<TYPE> {
    ...
    public ModuleEnumData(TYPE def) {
        ...
        this.enumConstants = Arrays.asList(this.defaultValue.getDeclaringClass().getEnumConstants());
    }
    ...
    public ModuleEnumData(TYPE def, int selectableCount) {
        ...
        if (constants.length < selectableCount) {
            ...
            this.enumConstants =  Arrays.asList(constants);
        } else {
            ...
            this.enumConstants =  Arrays.asList(constants).subList(0, selectableCount);
        }
    }
    ...
    public void write(String name, NBTTagCompound tag) {
        Objects.requireNonNull(tag, "Tag cannot be null.");
        Objects.requireNonNull(name, "Name cannot be null.");
        tag.setInteger(name, value.ordinal());
    }
}